...
static MtmElectionsResult mtmElectionsMayorOfCityAux(MtmElections mtmElections,
        City city,Id mayor, RankFunc rank, void* auxilaryData);
...
UniqueOrderedList mtmElectionsPerformElections(MtmElections mtmElections,
    ...
    RankFunc rank_function = rank;
    if (rank_function == DOES_NOT_EXIST)
        rank_function = mtmElectionsRankWithoutVoteVolume;
    Id mayor_id = (Id) malloc(sizeof(int));
    if (mayor_id == DOES_NOT_EXIST)
        return DOES_NOT_EXIST;
    ...
    SET_FOREACH(City, city, mtmElections->cities) {
       MtmElectionsResult city_result= mtmElectionsMayorOfCityAux(mtmElections,
               city, mayor_id, rank_function,auxilaryData);
        ...
        Vote vote=VoteCreate();
        VoteInsertInformation(vote,*mayor_id,
                              CitizenGetInformation(mayor,CITIZEN_NAME),
                              *mayor_id,
                              *(int*)CitizenGetInformation(mayor,CITIZEN_EDUCATION_YEARS),
                              *(int*) CitizenGetInformation(mayor,CITIZEN_AGE));
        if(uniqueOrderedListInsert(mayors,vote)
...
MtmElectionsResult mtmElectionsMayorOfCity(MtmElections mtmElections,
   ...
   // CityResult city_votes_result=
    return mtmElectionsMayorOfCityAux(mtmElections,city,
            mayor,mtmElectionsRankWithoutVoteVolume,&minimum_voter_age);
            CityMayorOfCity(city,mayor);
  /*  switch (city_votes_result){
        case CITY_NO_CANDIDATES_IN_CITY :
            return MTM_ELECTIONS_NO_CANDIDATES_IN_CITY;
        case CITY_MEMORY_ERROR:
            return MTM_ELECTIONS_MEMORY_ERROR;
...
static MtmElectionsResult mtmElectionsMayorOfCityAux(MtmElections mtmElections,
    ...
    SET_FOREACH(Citizen,citizen,CityGetCitizens(city)){//get citizens
      ...
      //  int candidate_votes=VoteCandidateGetVotes(old_vote);
      //  printf("votessofarforhim:%d",candidate_votes);
